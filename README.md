...
历史统计直接复用 CodexBar 的去重和计价逻辑，包括分叉任务继承历史、Fast 和长上下文。v0.1.11 会自动校正旧版重复计数及金额偏差；首次整理会显示进度，不影响账户与重置记录。比较 CodexBar 时须使用相同日期范围、分日时区及采样时刻。
...
计算页的结果/方法/原始数据和账户切换，统一使用“用量与目标”同款分段控件。建议主线采用更易读的字号，操作仍横排一行，保存结果不会挤动其他主线。
