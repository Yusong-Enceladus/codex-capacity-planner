...
历史统计直接复用 CodexBar 的去重和计价逻辑，包括分叉任务继承历史、Fast 和长上下文。整理只在真正打开“用量与目标”时按需运行，每次仅处理当前选择的日期范围与分日时区；本机记录没有变化时不启动全量扫描，仍在写入的日志也不会在后台自行循环追赶。低电量模式保留已确认历史但暂停重型整理，恢复供电后再按需更新。未整理完的日期不会被当作零用量，历史校正也不影响账户与重置记录。比较 CodexBar 时须使用相同日期范围、分日时区及采样时刻。
...
计算页的结果/方法/原始数据和账户切换，统一使用“用量与目标”同款分段控件。五个一级页面共用标题、正文和辅助文字的字号层级。计算图采用原生图表、对齐的百分比刻度和共用的时间轴，真实断档与账户周期切换处不会连线。建议主线的操作仍横排一行，保存结果不会挤动其他主线。
...
本机已确认刷新后，迟到的公告会与原有记录对账，不会把新额度继续按旧公告要求立刻用完。“官方已经执行”和“某个账号已确认到账”分别显示：缺少证据的账号标为未确认，不冒充 2/2。完成消息不会被当成另一次未来重置；公开预测若仍沿用刷新前的时间，原始数字保留供查看，暂不用于加速。真正独立的后续重置承诺仍会参与规划。
