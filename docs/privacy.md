...
- local aggregate token and model counters;
- bounded task metadata used for suggestions: title, project basename, recent
  activity, pin state, Goal state, and aggregate token change.

It does not need prompt text, response text, source code, tool output, full
project paths, authentication tokens, or browser cookies.
