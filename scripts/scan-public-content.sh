...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  scan_paths="README.md CodexResetApp codex-reset.js codex-reset-monitor.js codex-reset-behavior.js codex-reset-short-load.js codex-reset-workload-eval.js codex-reset.test.js receiver docs scripts patches"
fi
...
secret_files=$(rg -l --hidden -g '!patches/**/*.patch' -e "$secret_pattern" $scan_paths 2>/dev/null || true)
...
private_path_files=$(rg -l --hidden -e "$private_user_path|$temporary_item_path" $scan_paths 2>/dev/null || true)
