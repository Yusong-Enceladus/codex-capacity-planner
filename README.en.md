...
History directly reuses CodexBar's deduplication and pricing, including inherited fork prefixes, Fast and long contexts. An isolated background collector backfills older records, prioritizing the calendar that is furthest behind. Progress appears directly below the date controls; unfinished dates are not treated as zero usage. Historical corrections preserve account and reset records. Compare with CodexBar using the same date range, calendar time zone and sampling time.
