...
History directly reuses CodexBar's deduplication and pricing, including inherited fork prefixes, Fast and long contexts. Collection runs on demand only after Usage & Targets is actually opened and processes just the selected date range and calendar. Unchanged local records do not start a full scan, and an actively written log cannot create a background retry loop. Low Power Mode keeps confirmed history visible while deferring heavy collection until power is restored. Unfinished dates are not treated as zero usage, and historical corrections preserve account and reset records. Compare with CodexBar using the same date range, calendar time zone and sampling time.
...
Results, Method, Raw Data and the calculation account selector use the same native segmented controls as Usage & Targets. All five first-level pages share title, body and secondary-text sizes. Native calculation charts align percentage scales and share a time axis, without connecting across missing data or account-cycle boundaries. Mainline actions still share one row and saved feedback does not move neighbouring mainlines.
